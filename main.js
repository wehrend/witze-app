...
import { getStoredJokes, removeJoke, storeJoke } from "./storing";
...
function removeStoredJoke(index) {
  removeJoke(index);
  renderStoredJokes();
}

window.removeStoredJoke = removeStoredJoke;
  ...
  jokesToRender.forEach((joke, index) => {
    html += `
      <div class="joke-list__joke">
        ...
        <button class="joke-list__remove" onclick= "removeStoredJoke(${index})">
